...
async def run_presence_rotator(bot: discord.Client):
    ...
    try:
        ...
        log.debug("[Presence] Presence rotator task cancelled gracefully.")
        raise
    except Exception as e:
        log.error("[Presence] Fatal error in presence rotator: %s", e, exc_info=True)

def start_presence_task(bot: discord.Client) -> Optional[asyncio.Task]:
    """Spawns the presence rotator background task on the bot's event loop."""
    try:
        loop = getattr(bot, "loop", None)
        if loop is not None and loop.is_running():
            return loop.create_task(run_presence_rotator(bot))
    except Exception as e:
        log.debug("[Presence] Could not spawn presence rotator task: %s", e)
    return None
